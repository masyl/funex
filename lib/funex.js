...
*/
	...
	var tokenHandlers = {
		...
		"callOpen": function (chr, token) {
			if (chr != "(" || (token.length == 1)) return [__default];
		},
		"callClose": function (chr, token) {
			if (chr != ")" || (token.length == 1)) return [__default];
		},
		"arrayOpen": function (chr, token) {
			if (chr != "[" || (token.length == 1)) return [__default];
		},
		"arrayClose": function (chr, token) {
			if (chr != "]" || (token.length == 1)) return [__default];
		},
		"statementSeparator": function (chr, token) {
			if (chr != ";" || (token.length == 1)) return [__default];
		},
		"dot": function (chr, token) {
			if (chr != "." || (token.length == 1)) return [__default];
		},
		"argsSeparator": function (chr, token) {
			if (chr != "," || (token.length == 1)) return [__default];
		},
		"whitespace": function (chr, token) {
			// todo: add other whitespace codes
			if (__charMapWhiteSpace.indexOf(chr) < 0) return [__default];
		},
		"string": function (chr, token, exp, i) {
			var _chr = chr;
			var _token = token;
			// If the last char is a "'""
			if (chr == "'") {
