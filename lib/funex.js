(function (global) {
	...
	var __callOpen = "callOpen";
	var __callClose = "callClose";
	var __arrayOpen = "arrayOpen";
	var __arrayClose = "arrayClose";
	var __dot = "dot";
	var __argsSeparator = "argsSeparator";
	var __whitespace = "whitespace";
	var __string = "string";
	var __numeric = "numeric";
	var __name = "name";
	var __statementSeparator = "statementSeparator";
	...
	function executeTokens(tokens, context) {
		...
		for (i = 0; i < tokens.length; i++) {
			...
			switch (state) {
				...
				case  __callOpen:
					if (s0[0] === context[0]) // Prevent a function call on undefined
				...
				case __arrayClose:
					...
					if (s0[0] === undef)
						throw "Cannot read property " + tokens[i-1][0] + " of undefined : " + parsedStr;
					// Read the new value
					value = s0[0][ value[0] ];
					s0[0] = value;
					break;
				...
				case __string:
					s0.d = true; // Mark the context as dirty
					s0[0] = value = tokenStr.substring(1, tokenStr.length-1);
					break;
				case "json":
					s0.d = true; // Mark the context as dirty
					s0[0] = value = JSON.parse(tokenStr);
					break;
				case __numeric:
					s0.d = true; // Mark the context as dirty
	...
	var tokenHandlers = {
		...
		"string": function (cursor, chr, token, exp, i) {
			var _chr = chr;
			var _token = token;
		...
		"json": function (cursor, chr, token, exp, i) {
			...
			if (chr == "}") cursor.braces--;
			if (cursor.braces === 0)
				return [__default, "", token + chr, i + 1];
		},
			...
			if (tokenHandlerResult && tokenHandlerResult.length > 0) {
				...
				if (tokenHandlerResult.length > 1) {
					...
					if (tokenHandlerResult.length > 2) {
						...
						// if (tokenHandlerResult.length > 3) {
						// 	i = tokenHandlerResult[3];
						// }
					}
				}
			}
