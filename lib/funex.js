(function () {
	...
	function tokenizer(exp) {
		...
		if (exp.trim().length == 0){
			token = "''";
			if (token.length)
				instructions.push([token, __string]);
			return instructions;
		}
			...
			switch (state) {
				...
				case __string:	
					...
					if (chr == "'") {
						...
						//when a string in an expression starts with a double apostrophe, then token length
						// can be 1, but the token will be "'", so we can test that situation here as well
